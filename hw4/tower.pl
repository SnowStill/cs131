...
unique_list2(_, _, [], []).
unique_list2(Lper, N, [TH|TT], [CH|CT]) :-
    length(TH, N),
    permutation(Lper, TH),
    between(1,N, CH),
    check_count2(TH, 0, CH),
    unique_list2(Lper, N, TT, CT).
...
tower_time(Ttime) :-
    ...
    tower(5,_T,counts([1,3,2,3,2],[3,1,2,2,2],[1,3,5,2,2],[2,3,1,2,3])),
    statistics(cpu_time,[End|_]),
    Ttime is End - Start.

plain_tower_time(Ptime) :-
    statistics(cpu_time,[Start|_]),
    plain_tower(5,_T,counts([1,3,2,3,2],[3,1,2,2,2],[1,3,5,2,2],[2,3,1,2,3])),
    statistics(cpu_time,[End|_]),
    Ptime is End - Start.
